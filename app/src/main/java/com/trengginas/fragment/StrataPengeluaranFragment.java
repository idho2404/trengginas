...
public class StrataPengeluaranFragment extends Fragment {
    ...
    public StrataPengeluaranFragment() {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_strata_pengeluaran, container, false);
        ...
        if(isConnected()) {
            ...
            intent.putExtra("origin", "StrataPengeluaranFragment");
    ...
    private void loadTahunData() {
        Retrofit retrofit = ApiClient.getRetrofitInstance();
        Api api = retrofit.create(Api.class);
        Call<String> call = api.getTahunList("trengginas", "SELECT tahun FROM 0005_tahun WHERE kesra=1");
        call.enqueue(new Callback<String>() {
            @Override
            public void onResponse(Call<String> call, Response<String> response) {
                if (response.isSuccessful() && response.body() != null) {
    ...
    private void loadWebViewContent() {
        ...
        String url = "https://bpstrenggalek.com/trengginas/04_pdrbtabel_p.php?jdl=" + selectedNmtbl + "&th=" + selectedTahun;
        if(isConnected()) {
            webView.loadUrl(url);
        }else {
            Intent intent = new Intent(getActivity(), NetworkCheckActivity.class);
